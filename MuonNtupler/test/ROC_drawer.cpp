  ...
  TGraph * ROC_Drawer::graph_ROC(string var, int Nbins, float xmin, float xmax)
{
  ...
  TTree *tree_bkg = (TTree*)file_bkg.Get("ntupler/MuonTree");
  TTree *tree_sig = (TTree*)file_sig.Get("ntupler/MuonTree");
...
//gPad->SetGrid();
  ...
  TGraph * gr1 =  graph_ROC("relIsoWithDBeta", 1000, 0., 15.);
  TGraph * gr2 =  graph_ROC("relIso_PUPPI", 1000, 0., 15.);
  ...
  leg->AddEntry(gr1, "relIsoWithDBeta","l");
