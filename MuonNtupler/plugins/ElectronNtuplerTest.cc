// -*- C++ -*-
//
// Package:    ElectronWork/ElectronNtuplerTest
// Class:      ElectronNtuplerTest
// 
/**\class ElectronNtuplerTest ElectronNtuplerTest.cc ElectronWork/ElectronNtuplerTest/plugins/ElectronNtuplerTest.cc
...
class ElectronNtuplerTest : public edm::EDAnalyzer {
public:
  explicit ElectronNtuplerTest(const edm::ParameterSet&);
  ~ElectronNtuplerTest();
...
private:
  ...
  Float_t relIsoWithEA_MapBasedVeto;
...
//
ElectronNtuplerTest::ElectronNtuplerTest(const edm::ParameterSet& iConfig):
...

{
  ...
  electronTree_ -> Branch("relIsoWithEA_MapBasedVeto", &relIsoWithEA_MapBasedVeto, "relIsoWithEA_MapBasedVeto/F");
  ...
  electronTree_ -> Branch("reliso_raw", &reliso_raw, "reliso_raw/F");
...
ElectronNtuplerTest::~ElectronNtuplerTest()
{
...
void
ElectronNtuplerTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace std;
  using namespace edm;
  ...
  for (unsigned int iElectron = 0; iElectron < electrons -> size(); iElectron++) {
    ...
    //CITK map based veto
    sumChargedHadronPt_CITK_MapBasedVeto =  (*ValueMaps_ChargedHadrons_MapBasedVeto)[elePtr];
    sumNeutralHadronPt_CITK_MapBasedVeto =  (*ValueMaps_NeutralHadrons_MapBasedVeto)[elePtr];
    sumPhotonPt_CITK_MapBasedVeto        =  (*ValueMaps_Photons_MapBasedVeto)[elePtr];
    ...
    reliso_raw = (sumChargedHadronPt_CITK + sumNeutralHadronPt_CITK + sumPhotonPt_CITK)/pt_;

    relIsoWithEA_MapBasedVeto = (sumChargedHadronPt_CITK_MapBasedVeto + sumNeutralHadronPt_CITK_MapBasedVeto + sumPhotonPt_CITK_MapBasedVeto - rho_ * area )/pt_;
...
void 
ElectronNtuplerTest::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
ElectronNtuplerTest::endJob() 
{
}
...
/*
void 
ElectronNtuplerTest::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
ElectronNtuplerTest::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
ElectronNtuplerTest::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
ElectronNtuplerTest::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
ElectronNtuplerTest::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool ElectronNtuplerTest::checkAncestor(const reco::Candidate *gen, int ancestorPid){

  // General sanity check
  if( gen == 0 ){
    printf("ElectronNtuplerTest::checkAncestor: ERROR null particle is passed in, ignore it.\n");
    return false;
  }
...
// The function that uses algorith from Josh Bendavid with 
// an explicit loop over gen particles. 
int ElectronNtuplerTest::matchToTruth(const pat::Electron &el, 
  ...
  if( ancestorPID == -999 && ancestorStatus == -999 ){
    ...
    // Complain.
    printf("ElectronNtuplerTest: ERROR! Electron does not apper to have a non-electron parent\n");
    return UNMATCHED;
  }
...
void ElectronNtuplerTest::findFirstNonElectronMother(const reco::Candidate *particle,
						 int &ancestorPID, int &ancestorStatus){

  if( particle == 0 ){
    printf("ElectronNtuplerTest: ERROR! null candidate pointer, this should never happen\n");
    return;
  }
...
// The function that uses the standard genParticle() matching for electrons.
int ElectronNtuplerTest::matchToTruthAlternative(const pat::Electron &el){
...
void ElectronNtuplerTest::printAllZeroMothers(const reco::Candidate *particle){
  
  if( particle == 0 ){
    printf("ElectronNtuplerTest::printAllZeroMothers: reached the top of the decay tree\n");
    return;
  }
  
  printf("ElectronNtuplerTest::printAllZeroMothers: ancestor ID= %d, status= %d\n",
...
DEFINE_FWK_MODULE(ElectronNtuplerTest);
